...
const ImageWithPlaceholder: React.FC<ImageWithPlaceholderProps> = ({ src, alt, className, placeholderClassName, ...props }) => {
  ...
  return (
    <div className="relative w-full h-full bg-gray-200">
      {showPlaceholder && (
        <div className={`absolute inset-0 flex items-center justify-center text-gray-400 ${placeholderClassName}`}>
          <PhotoIcon className="w-1/4 h-1/4 min-w-[24px] min-h-[24px]" />
        </div>
      )}
      
      {showError && (
        <div className={`absolute inset-0 flex items-center justify-center bg-gray-100 text-gray-500 ${placeholderClassName}`}>
