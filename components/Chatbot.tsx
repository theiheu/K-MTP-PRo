...
const ChatBubbleIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
  ...
  >
    <path
      ...
      d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.056 3 12c0 2.228.87 4.25 2.28 5.834a8.96 8.96 0 0 0 1.898 2.05A9.013 9.013 0 0 0 9 21.75c1.112 0 2.18-.24 3.162-.682A8.95 8.95 0 0 0 12 20.25Z"
    />
  </svg>
);
...
const Chatbot: React.FC<ChatbotProps> = ({ allProducts }) => {
  ...
  useEffect(() => {
    const updatePosition = () => {
      ...
      if (savedPosition) {
        ...
        const buttonSize = 80;
        setPosition({
          x: window.innerWidth - buttonSize - 16, // 16px margin from edge
          y: window.innerHeight - buttonSize - 24, // 24px bottom margin
        });
      }
    };
  ...
  const handleSend = async () => {
    ...
    try {
      const { recommendedProducts, message, responseType } =
        await getAIRecommendations(inputValue, allProducts);
      ...
      let errorMessage = "Xin lỗi, tôi đã gặp lỗi. ";

      if (error instanceof Error) {
        if (error.message.includes("API_KEY")) {
          errorMessage += "Hệ thống AI tạm thời không khả dụng.";
  ...
  return (
    <>
      <div
        ...
        className={`z-50 transition-opacity duration-300 ease-out ${
      ...
      <div
        className={`fixed bottom-[-25] right-0 sm:m-6 lg:m-8 w-full sm:max-w-md h-full sm:h-[70vh] flex flex-col bg-white rounded-t-lg sm:rounded-lg shadow-2xl transition-transform duration-300 ease-out ${
          ...
          {messages.length === 0 && !isLoading ? (
            <div className="flex flex-col h-full items-center justify-center text-center text-gray-500 p-4">
              ...
              <p className="mt-4 text-sm">
                Tôi có thể giúp gì cho bạn? <br /> Hãy bắt đầu bằng cách nhập
                câu hỏi của bạn vào bên dưới.
              </p>
            </div>
          ) : (
                  ...
                  {msg.products && msg.products.length > 0 && (
                    <div className="mt-2 space-y-2 border-t border-gray-300 pt-2">
                      {msg.products.map((p) => (
                        <div
                          key={p.id}
                          className="flex items-center gap-2 text-xs p-1 bg-white rounded"
                        >
