...
const AdminPage: React.FC<AdminPageProps> = ({ products, categories, onAddProduct, onUpdateProduct, onDeleteProduct, onAddCategory, onDeleteCategory, onUpdateCategory, onReorderCategories }) => {
  ...
  const handleCategoryFormSubmit = async ({ originalName, data }: { originalName?: string; data: { name: string; iconFile: File | null; icon: string } }) => {
    try {
        ...
        if (editingCategory && originalName) { // Editing
  ...
  const filteredProducts = useMemo(() => {
    const productsWithTotalStock = products.map(p => {
        const isComposite = p.variants.some(v => v.components && v.components.length > 0);
        let displayStock: number;

        if (isComposite) {
            const compositeVariants = p.variants.filter(v => v.components && v.components.length > 0);
            displayStock = compositeVariants.reduce((sum, v) => sum + calculateVariantStock(v, products), 0);
        } else {
      ...
      <div className="mt-6">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      <th scope="col" className="relative px-2 py-3"><span className="sr-only">Mở rộng</span></th>
                    ...
                    {filteredProducts.length > 0 ? (
                      filteredProducts.map((product) => (
                        <React.Fragment key={product.id}>
                          <tr onClick={() => handleToggleExpand(product.id)} className="cursor-pointer hover:bg-gray-50 transition-colors duration-150">
                            <td className="px-2 py-4">
                               <ChevronDownIcon className={`w-5 h-5 mx-auto text-gray-400 transform transition-transform duration-200 ${expandedProductId === product.id ? 'rotate-180' : ''}`} />
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center">
                                <div 
                                  className="flex-shrink-0 h-10 w-10 rounded-md cursor-pointer"
                                  onClick={(e) => { e.stopPropagation(); handleOpenGallery(product.images, 0); }}
                                >
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center gap-2">
                                ...
                                {product.isComposite && (
                                  <span className="text-xs font-semibold bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full">Bộ</span>
                                )}
                              </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-4">
                              <button onClick={(e) => { e.stopPropagation(); handleOpenEditModal(product); }} className="text-yellow-600 hover:text-yellow-800">Sửa</button>
                              <button onClick={(e) => { e.stopPropagation(); setProductToDelete(product); }} className="text-red-600 hover:text-red-900">Xóa</button>
                            </td>
                          </tr>

                          {expandedProductId === product.id && (
                            <tr>
                              <td colSpan={5} className="p-0 border-t-2 border-yellow-200">
                                <div className="p-4 bg-yellow-50">
                                  <h4 className="text-md font-semibold text-gray-700 mb-3 ml-2">Chi tiết Biến thể</h4>
                                  <div className="overflow-x-auto">
                                    <table className="min-w-full divide-y divide-gray-200 bg-white rounded-lg shadow-inner">
                                        <thead className="bg-gray-100">
                                            <tr>
                                                <th className="px-4 py-2 text-left text-xs font-medium text-gray-600 uppercase">Biến thể</th>
                                                <th className="px-4 py-2 text-left text-xs font-medium text-gray-600 uppercase">Tồn kho</th>
                                                <th className="px-4 py-2 text-left text-xs font-medium text-gray-600 uppercase">Thành phần</th>
                                                <th className="px-4 py-2 text-left text-xs font-medium text-gray-600 uppercase">Ảnh</th>
                                            </tr>
                                        </thead>
                                            ...
                                            {product.variants.map((variant: Variant) => (
                                                <tr key={variant.id}>
                                                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-800 align-top">
                                                        <p className="font-medium">{Object.values(variant.attributes).join(' / ') || 'Mặc định'}</p>
                                                        <div className="text-xs text-gray-500 mt-1">Đơn vị: {variant.unit || 'N/A'}</div>
                                                        <div className="text-xs text-gray-500">Giá: {variant.price != null ? `${variant.price.toLocaleString('vi-VN')} đ` : 'N/A'}</div>
                                                    </td>
                                                              ...
                                                              {variant.components.map(comp => {
                                                                  ...
                                                                  return (
                                                                      <li key={comp.variantId}>
                                                                          {comp.quantity} x {compVariant ? (Object.values(compVariant.attributes).join(' / ') || 'Mặc định') : `ID: ${comp.variantId}`}
                                                                      </li>
                                                                  )
                                                              })}
                                                          </ul>
                                                    ...
                                                    <td className="px-4 py-3 whitespace-nowrap align-top">
                                                      {(variant.images && variant.images.length > 0) ? (
                                                          <div className="h-10 w-10 rounded-md cursor-pointer" onClick={(e) => { e.stopPropagation(); handleOpenGallery(variant.images!, 0); }}>
                                                              <ImageWithPlaceholder src={variant.images[0]} alt="variant" className="h-full w-full object-cover rounded-md"/>
                                                          </div>
                                                      ) : (product.images.length > 0 ? (
                                                          <div className="h-10 w-10 rounded-md opacity-50" title="Sử dụng ảnh chung">
                                                            <ImageWithPlaceholder src={product.images[0]} alt="variant" className="h-full w-full object-cover rounded-md"/>
                                                          </div>
                                                        ) : '—'
                                                      )}
                                                    </td>
        ...
        {activeTab === 'categories' && (
          <div role="tabpanel" id="categories-panel">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-semibold text-gray-800">Danh sách Danh mục</h2>
                <button
             ...
             <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-center">
                <p className="text-sm text-gray-600">💡 Mẹo: Bạn có thể kéo và thả các danh mục bên dưới để sắp xếp lại thứ tự của chúng.</p>
            </div>
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {categories.map((cat, index) => (
                    ...
                    >
                        ...
                        <span className="text-sm font-medium text-gray-800 break-all h-10 flex items-center">{cat.name}</span>
                        <div className="mt-3 pt-3 border-t w-full flex justify-center space-x-3">
                            <button onClick={() => handleOpenEditCategoryModal(cat)} title="Sửa" className="text-gray-500 hover:text-yellow-600 p-1.5 rounded-full hover:bg-gray-100">
                                <PencilIcon className="w-5 h-5" />
                            </button>
                            <button onClick={() => setCategoryToDelete(cat)} title="Xóa" className="text-gray-500 hover:text-red-600 p-1.5 rounded-full hover:bg-gray-100">
