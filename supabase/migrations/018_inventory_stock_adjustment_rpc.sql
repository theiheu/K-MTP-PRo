-- Atomic stock adjustment RPC for inventory audits.
-- Creates the adjustment document, its items, adjustment movements, and balance updates in one transaction.
...
BEGIN
  IF p_items IS NULL OR jsonb_typeof(p_items) <> 'array' OR jsonb_array_length(p_items) = 0 THEN
    RAISE EXCEPTION 'Stock adjustment requires at least one item';
  END IF;
  ...
  IF v_warehouse_id IS NULL THEN
    RAISE EXCEPTION 'Stock adjustment requires a warehouse';
  END IF;

  v_document_code := next_inventory_document_code('stock_adjustment', p_document_date);
  ...
  LOOP
    v_delta := COALESCE(
      NULLIF(v_item->>'adjustmentDelta', '')::NUMERIC,
      NULLIF(v_item->'metadata'->>'adjustmentDelta', '')::NUMERIC
    );
    ...
    VALUES (
      ...
      v_quantity,
      v_quantity,
      CASE WHEN v_delta < 0 THEN v_quantity ELSE NULL END,
      CASE WHEN v_delta > 0 THEN v_quantity ELSE NULL END,
      v_item->>'unit',
      ...
      COALESCE(v_item->'metadata', '{}'::jsonb) || jsonb_build_object('adjustmentDelta', v_delta)
    )
  ...
  IF v_display_order = 0 THEN
    RAISE EXCEPTION 'Stock adjustment requires at least one non-zero item';
  END IF;
